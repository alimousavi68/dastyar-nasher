...
│   ├── monitoring_page.php      # صفحه مانیتورینگ منابع خبری و نمایش خطاهای فعال
...
│   │   ├── monitoring.php       # هسته منطق پایش، تست منابع و صف تلاش مجدد خطاها
...
graph TD
    ...
    K -->|اولویت بالا / متوسط / کم| M[ثبت اکشن تکی در Action Scheduler]
    
    N[Action Scheduler Queue] -->|اجرا بر اساس زمان‌بندی مستقل| O(فراخوانی اکشن i8_action_publish_specific_post)
    O -->|بررسی وضعیت پیش‌نویس و انتشار مستقیم| P[تغییر وضعیت نوشته به publish با زمان فعلی]
    
    Q[سیستم مهاجرت: admin_init] -->|مهاجرت خودکار داده‌های قدیمی| M
```
...
3. `{prefix}pc_post_schedule`: ذخیره صف انتشار پست‌های قدیمی (این جدول جهت حفظ سازگاری عقب‌رو و مهاجرت ایمن داده‌های قبلی حفظ شده است و ردیف‌های آن به تدریج به Action Scheduler منتقل می‌شوند).
4. `{prefix}pc_reports`: نگهداری گزارشات موفقیت یا شکست اسکرپ و وظایف خودکار سیستم.
5. `{prefix}i8_monitoring_errors`: ثبت و نگهداری خطاهای فعال، زمان وقوع و وضعیت تکرار مجدد تست منابع خبری.

### توابع شاخص سیستم:
* `i8_test_resource($resource_id)`: موتور اعتبارسنجی اتصالات RSS و ساختار قالب سلکتورهای خبرگزاری به صورت عاری از نشت حافظه.
* `i8_process_resource_check($resource_id)`: بررسی تکی وضعیت منبع و ثبت خطا یا پیشرفت موفقیت.
* `i8_retry_resource_monitoring($resource_id)`: مدیریت تلاش‌های مجدد (تا ۳ بار در فواصل ۱۰ دقیقه‌ای) برای خطاهای موقت جهت تفکیک از خطاهای دائمی.
...
* `calculate_post_publish_time()`: محاسبه دقیق فاصله زمانی انتشار پست‌ها بر مبنای ساعات کاری روزانه و تعداد خبر هدف روزانه.
* `i8_action_log($message)`: ثبت لاگ‌های ردیابی در سیستم بومی لاگر Action Scheduler برای مستندسازی زنده وضعیت پردازش صف.
* `i8_publish_specific_post_callback($post_id)`: کالبک انتشار اختصاصی پست به همراه بررسی ایمن وضعیت پیش‌نویس (Draft) و فارسی‌سازی خطاهای لاگ.
* `i8_get_next_available_publishing_slot()`: محاسبه نزدیک‌ترین اسلات زمانی خالی برای انتشار پست با حفظ اولویت‌ها و فواصل انتشار.
* `i8_adjust_timestamp_to_working_hours($timestamp)`: جابجایی هوشمند زمان انتشار پست‌ها به اولین اسلات خالی در ساعات کاری تعریف شده سایت.
* `i8_migrate_old_queue_to_action_scheduler()`: تابع انتقال خودکار و بدون تداخل داده‌های صف قدیمی جدول به سیستم مدرن اکشن اسکژولر.
* `i8_ensure_scheduled_action_exists()`: تضمین اجرای صف انتشار در رویداد `admin_init` و مدیریت فرآیند انتقال صف به Action Scheduler.
* `i8_setup_fallback_cron()`: راه اندازی مکانیزم محافظ و لایه دوم با استفاده از هوک‌های استاندارد WordPress Cron جهت نظارت و بازیابی زمان‌بندی در صورت غیرفعال شدن ناگهانی صف.
...
* **انتقال فرآیند انتشار به سیستم توزیع‌شده اکشن اسکژولر:** به منظور کاهش لود سرور و جلوگیری از پردازش‌های موازی سنگین، انتشار پست‌ها به صورت تسک‌های تکی و غیرهمزمان (Async) با فواصل محاسبه شده بر اساس ساعات کاری مدیریت می‌شود.
* **پایش منابع با استفاده از سیستم وظایف غیرهمزمان:** برای جلوگیری از اتمام زمان اجرای اسکریپت (Timeout) یا مسدودسازی وب‌سایت در زمان بررسی گروهی منابع خبری، فرآیند مانیتورینگ به صورت کامل بر بستر کارهای غیرهمزمان در Action Scheduler توزیع گردیده است.
* **پیشگیری فعال از نشت حافظه (Memory Leak):** در تمام تراکنش‌های تحلیلی و پارس ساختار HTML صفحات مقصد، منابع فیزیکی و بافرهای حافظه شیء DOM با متد `clear()` به سرعت تخلیه می‌گردند.
...
3. وضعیت صف را در صفحه **دستیار > صف انتشار** بررسی کنید و در صورت نیاز از ابزار بازسازی دستی صف جهت بازیابی خودکار استفاده کنید.
